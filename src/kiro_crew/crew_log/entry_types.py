...
_SESSION_TYPES: tuple[EntryType, ...] = (
    ...
    EntryType(
        ...
        note=(
            ...
            "folds under is the member's and no second crew can reach it. Each publish "
            "REPLACES the panel, so the fold takes the newest entry whole and keeps the "
            "earlier ones only as a short history; a partial update has no meaning here, "
            "unlike the ledger's. The panel therefore DEPENDS on this log: with the "
            "emitter off the publish refuses rather than keeping a document of its own."
        ),
    ),
)
