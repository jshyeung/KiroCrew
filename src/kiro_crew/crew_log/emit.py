...
def on_panel_published(session_id: str, data: dict[str, Any], *, timeout: float = 5.0) -> bool:
    ...
    WAITS, like ``on_work_recorded`` and for the same reason: the fold is the record,
    so the route may report success only for an entry that really landed. It returns
    ``True`` once the writer has appended, and ``False`` when the append was refused,
    permanently dropped, or not started within *timeout* seconds. ``False`` is FINAL
    -- an entry the waiter gave up on is abandoned and will not land later even if
    the writer retries the job. Without that, a slow store could answer the crew 503
    and then commit anyway, leaving the drawer stale and making the crew's retry add
    a second history row for one publish. An append already STARTED is waited to
