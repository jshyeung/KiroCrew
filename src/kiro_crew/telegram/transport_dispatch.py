...
class TelegramDispatcher:
    ...
    async def _handle_busy(
        ...
        *privacy_request* is a modifier the caller stripped off *text*, and the two
        ...
        mark and header, strictly (``privacy_mode.reserve``) -- or the running turn's
        ...
        if mode != "queue" and not msg.attachments:
            ...
            if privacy_request and can_steer:
                ...
                # would both steer). reserve applies strictly -- a row that cannot
                # be taken or written is a refusal, already audited and announced:
                # no steer, nothing runs -- and hands back what a failed steer
                # must release.
                try:
