...
def _scrub_decoded(val: object, depth: int = 0) -> object:
    ...
    if isinstance(val, dict):
        ...
        for key, item in val.items():
            name = (
                _scrub_json_transport(_scrub_text(key), depth + 1) if isinstance(key, str) else key
            )
            if name in cleaned:
                raise _UnscannableJSON
...
def _scrub_json_transport(val: str, depth: int = 0) -> str:
    ...
    The dividing line is the PARSER's own verdict, not any failure while decoding. Text the
    parser rejects as malformed holds no JSON payload at all, so the text scrub already
    covered everything there was to cover and the field passes through. Every other failure
    means the text IS JSON and decoding still could not finish -- nested past the cap,
    nested deeply enough to exhaust the parser's own recursion, repeating an object name so
    decoding discards a member, holding names that collide once scrubbed, or tripping a
    limit of this interpreter that a consumer's parser does not share, such as the cap on
    converting a very long integer. Such a document has a payload this chain cannot
    certify, and shipping an uncertified payload is the hole being closed here, so the
    field is withheld and a marker goes out instead.
    """
