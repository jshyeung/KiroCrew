...
def _type_carries_a_credential(event_type: str) -> bool:
    """True when the type's own text carries a credential or an exfiltration URL.

    The type is REFUSED rather than redacted, because it is an identifier: the
    grant patterns match on it and every envelope stores it verbatim, so a
    rewritten type would stop matching the pattern its contributor declared and
    would land in the log as a different event than the one that was appended.

    Detection is the same chain the egress paths run over ``data``, so the two
    boundaries cannot disagree about what a credential looks like.
    """
    return _redact_projection_value(event_type) != event_type
...
async def api_eventlog_events_post(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # Its content is bounded here too, on the same ground: the namespace
        # pattern fixes the type's SHAPE -- one slash, a non-reserved namespace --
        # and leaves the name segment free, while the egress redactors scrub only
        # `data`. A credential spelled into the name would therefore reach storage
        # and every co-subscribed app unredacted.
        if _type_carries_a_credential(event_type):
            raise ContribError(
                "invalid_event_type",
                "event type must not contain a credential or a presigned URL: it "
                "is stored verbatim and is never redacted on the way out",
            )
        if not grants.may_use_kind(app, kind):
