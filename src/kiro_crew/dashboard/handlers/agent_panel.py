...
async def api_agent_panel_publish(request: web.Request) -> web.Response:
    ...
    if crew_log_emit.enabled():
        ...
        if not await asyncio.to_thread(_append):
            # Logged, not returned: the panel itself is published and readable.
            logger.warning("panel history not recorded for crew %s; the record was written", slug)
...
def _panel_record(slot: str, slug: str, owner_key: str) -> dict[str, Any] | None:
    ...
    THE FOLD IS PREFERRED, not authoritative. It is read for *slot* -- the member's
    own DM slot, which is the only slot a publish can append under -- through the
    ordinary slot-keyed projection, so it is served by the same warm kernel every
    other slot fold uses and this route keeps no cache of its own. It is preferred
    because it carries what the file cannot: the publish history, and one record per
    owner.
    ...
    *owner_key* selects WHICH record, because one slot can carry two crews: the slot
    ...
    published last -- which the single file cannot do.

    The file is the DURABLE record, and the reason this read has a second source at
    all. It answers for a crew that published with the crew log off, for one that
    published before this entry type existed, and for one whose session unit has
    since been collected by retention -- a panel outlives any single session, so a
    fold-only read would blank a webview that is still on disk. Nothing here writes
    it: this is a GET, and the store owns that write.
