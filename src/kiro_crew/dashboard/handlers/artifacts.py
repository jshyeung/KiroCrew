...
async def api_artifact_delete(request: web.Request) -> web.Response:
    ...
    try:
        ...
        #
        # Only for a slug the store RESOLVED above. Guards are keyed by slug and kept for
        # the life of the process, so minting one from the request path would let a stream
        # of unknown or malformed slugs grow that table without bound -- this route reaches
        # `delete` with either, deliberately, so the store can answer 4xx. A slug naming no
        # artifact also has no publish to race: `publish` reads the artifact inside the
        # guard and fails when it is absent. What that leaves uncovered is one narrow
        # order -- an artifact created AND first-published entirely inside this window, on
        # a slug that did not exist when this request read the store -- which is pinned by
        # a test rather than paid for with an unbounded table.
        #
        ...
        # is where it becomes decisive. `None` when the store resolved nothing above --
        # there is no generation to pin, and `delete` reports the absence itself.
        ...
        # Under the publication guard for ANY well-formed slug, including one the store
        ...
        async with AsyncExitStack() as guard:
            ...
            await _run_off_loop(
                lambda: get_default_store().delete(
                    ...
                    expect_created_at=(None if _existing is None else _existing.created_at),
                )
            )
        ...
        # The replacement arrived after the 409 check above and before this lock, so the
        # same answer is owed: its copy is not this request's to take down, and the
        # removal has no undo.
        msg = (
            "This artifact was deleted and a new one created under the same name "
            "while this delete was running. The new artifact was left alone. Delete "
            "it again if you meant to remove the replacement too."
        )
        _audit(
            tool="artifact_delete",
            request=request,
