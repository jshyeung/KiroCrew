...
def auth_store_staging_dir(home: Path) -> Path:
    ...
    * group- or world-READABLE or executable on POSIX -- narrowed, and WARNED when the
    ...
    Refusing costs no persisted state, because each caller reaching a refusal falls back to
    publishing through the target's own directory rather than dropping the write: the
    signing key through its in-place writer, the refresh state through the target's parent.
