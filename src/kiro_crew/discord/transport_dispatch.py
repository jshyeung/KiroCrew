...
class DiscordDispatcher:
    ...
    async def deliver_spawn_approval(
        ...
        if client is None:
            return None
        if not await channel_inbound_permitted("discord"):
            # The operator's channels ceiling, which the startup gate cannot
            # speak for: a host profile that denies this channel after the
            # transport connected leaves it up and dispatching. Checked FIRST,
            # ahead of the nonce arm and any DM-channel open, because a denied
            # channel also drops the Approve press that would answer the prompt
            # (only an explicit reject is exempt there) — so posting anyway
            # strands the wait until it deny-by-defaults, and the gate takes that
            # elapsed wait for a real decision. Falling through instead keeps the
            # spawn answerable on Slack/dashboard.
            logger.info(
                "Discord: not posting the spawn-approval prompt for %s; the "
                "channel is denied by channels governance policy",
                request_id,
            )
            return None
