...
def _vet_app_owner_enabled(job: CronJob) -> str | None:
    ...
    Only a definite ``True`` authorizes. An unreadable state (None) is no licence
    to run an app's code: ``apps.backend`` reads it the same closed way before
    spawning one, reserving ``is not False`` for callers whose action -- deleting
    files -- cannot be undone. A skip is recoverable and persists nothing, so the
    next fire re-asks and a readable enabled state resumes the job.
    """
    ...
    verdict = "is disabled" if state is False else "has an unreadable enabled state"
    logger.warning(
