...
| any backend | `cc` | hidden, `config` re-exposed | visible |
...
leaves `known_hosts` readable. The last row is the adapter mask. A harness whose
...
Linux-launcher primitive; the macOS seatbelt builder needs an equivalent rule
before phase 1 closes there, and Windows has no bind mask at all — see Scope.
...
tier, and so does every harness on `cc` or `strict` — with one exception the
predicate must honour. The mask it reads is the one the launcher ACTUALLY
...
mask covers: an `AF_UNIX` socket on POSIX, a named pipe on Windows, started with
the gateway whether or not anything else is. The socket file is `0600` inside a
`0700` directory; the pipe carries an ACL naming only the operator's SID. It is not
...
**A helper and two env entries, for the child.** A `0700` helper script in the
granted leaf, written the way `_ensure_pi_gate_launcher` writes the pi launcher,
that dials the AWS route and prints the answer; a config file beside it whose only
content is a `credential_process` line naming the helper; and in the child's
...
- `SSH_AUTH_SOCK` is scrubbed TWICE today, and both scrubs must let the proxy
  path through. The parent-side scrub (`scrub_agent_subprocess_env`) and the
  launcher-side scrub — the Linux launcher's in-namespace unset loop and the
  macOS seatbelt's `env -u` list — both take the one boolean
  `_forward_ssh_auth_sock` resolves and both drop `"SSH_AUTH_SOCK"` from their
  prefix list only when it is true. The proxy therefore needs that boolean (or a
...
```
masked child ─┬─ AWS SDK ─> helper (grant leaf) ──bearer──> AWS vend listener ──> host credential chain
              │                                              (runtime_dir, shared)
...
the child private keys, and the module docstring of `acp/harness/codex.py` records
that trade being made the other way on purpose.
...
pod remap. Platforms: the AWS route on every platform; the per-session leaf and
the SSH proxy on Linux in phase 1–3, macOS once the seatbelt builder has a
per-spawn window rule, Windows for neither (no bind mask, no `SSH_AUTH_SOCK`).
...
| 3 | the helper, the config file, the env wiring | a masked session resolves AWS credentials on every platform, and signs over SSH on Linux, with no operator step; the proxy path survives both scrubs; an operator-set `AWS_CONFIG_FILE` still wins; a granted consent leaf still forwards the raw socket; a delegated `kiro-cli` spawn gets no grant |

## Decisions needed

Each is a posture ruling, not a code shape. Phase 2 waits on all three.
...
3. **Proxy scope.** Whether the proxy can restrict signing to keys whose comment
   ...
   false refusals.
4. **SSH on Windows.** Win32 OpenSSH selects its agent by a fixed named pipe, not
   `SSH_AUTH_SOCK`. Whether Crew can stand in for that pipe per session, or
   whether Windows keeps today's behaviour, is undecided.
...
permissions against other users (`0600` in a `0700` directory, or a pipe ACL on
the operator's SID) and by a bearer on every request against a same-uid caller;
