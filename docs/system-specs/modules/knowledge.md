...
- **A bundle carries per-document OWNERSHIP with the content** — `export_all` ships `folder_file_state`, `artifact_item_state` and `agent_item_state` (`store.BUNDLE_STATE_KEY_COL` is the one list, derived from `_DOC_STATE_TABLES`, that the importer and the dashboard's bundle validator both read). An item with no state row is unmanaged: nothing claims it for de-duplication, the Sources UI has no per-document group label for it, and a later ingest of the same document adds a second copy instead of replacing the first. A row is restored only when its WHOLE group is in the bundle AND landed under the mapped source, and it is written with that table's own live status (`done` for folder, `active` for the aggregates — the vocabularies are not interchangeable). A row that does not qualify is dropped and its items arrive unowned: an empty group is a dedup claim or a scan marker describing the exporting store's progress, and the next ingest of that document re-derives it.
- **A local state row wins a document-key collision only while it OWNS live items, and then the bundle's copy of that document does not arrive at all** — a `(source, key)` pair IS a document's identity within a source, so two documents cannot share one. A local row whose group is empty or names only deleted items is holding a marker (a `deduped` loser, a pending or failed scan), not ownership, so the imported row takes the key; its claim on another source's items is released with it through `detach_source_location_by_hash`, because a claim left behind under a hash no row names any more means a later deletion of the holder reassigns an item into this source with nothing to adopt it into. A row that DOES own items keeps the key, and `_bundle_blocked_items` then withholds that document's items from the import entirely — importing them anyway would leave them permanently unowned, which is the defect this round-trip exists to remove, not a lesser version of it. That is the only refusal reason that blocks an item: every other one rejects a ROW as an untrustworthy statement about items that are themselves legitimate, and those items arrive unowned exactly as they do from a bundle carrying no state tables.
...
- **`folder_file_state.mtime` does not travel; the content hashes do** — the folder scan skips hashing a live row whose recorded mtime is at or above the file's (`folder_watcher` `status in ('done','deduped') and mtime <= state['mtime']`), so a carried mtime from the exporting host would stop the local file ever being hashed again, silently leaving the imported text indexed in place of the local content with no self-correcting path. The importer writes a fixed floor below any timestamp a watched file carries, so the first local scan always hashes, and `content_hash` / `text_hash` — which DO travel — are what keep that cheap by skipping re-extraction on a match. The bundle's `mtime` is not validated, because nothing reads it.
