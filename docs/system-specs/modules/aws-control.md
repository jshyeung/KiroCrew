...
A stop signal is success only when the sidecar's final cycle also committed. That
...
mid-upload -- exits non-zero, and so does a status that could not be read. A task
with no bucket has no sidecar and nothing to weigh.
