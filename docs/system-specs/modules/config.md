...
`config.json` is a `READONLY` sandbox leaf, so the agent's shell cannot write it, but
it is off the read+write floor and the agent can still READ it, and every
