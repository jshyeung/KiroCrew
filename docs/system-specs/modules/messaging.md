...
- **`apply_mode(mode, session_key, *, source, caller, resources, sessions, notify,
  ...
  header write, the caller's `on_applied` hook, and the notice. The map's
  debounced write is AWAITED (`aflush`) right after the mark and the audit, in
  both forms, so nothing published past that point -- header, hook, notice,
  return -- claims a row that has not landed; the audit stays ahead of that
  await, so a task cancelled mid-write has recorded the mode. On a failed write
  the two forms part: the best-effort form keeps the mark -- the session stays
  restricted for this process, the safe direction -- and logs the write it is
  still owed (telling the user privacy is off while it is on would be the worse
  error); a persist FAILURE at `set_flag` is likewise logged, not raised. A
  persist REFUSAL is different: when the map declines to
...
- **`reserve(mode, session_key, …) -> Reservation`**, **`commit(reservation)`**,
  **`release(reservation, *, sessions, source, …)`** — the form for a caller about
  ...
  before it returns — a write the map cannot perform, at `set_flag` or at the
  awaited flush, is a refusal (`persist_failed`, the flush failure taking the mark
  and the flag back with it and the trail reading `allowed` then `denied`) rather
  than the best-effort mark, because the step would run on the strength of the
  row (the Telegram steer folds the message into a turn that persists its
  transcript). The shared holder for a
  ...
  landed step `commit`s; a step that did not land `release`s, durable first and
  published last, the mirror of the apply: the map flag is cleared and its write
  AWAITED, the transcript header restored to what it recorded before (only if it
  still records what the stamp wrote), and only then is the tracker mark dropped
  and the reversal reported, one SEL `released` record beside the `allowed` one.
  A clear that cannot reach disk RETAINS the mode, fail-closed toward private:
  the flag goes back into the map (its next write retries), the mark never left,
  the header still says the mode, and one `retained` record
  (`release_failed:persist_failed:<target>`) reports the failure in place of
  `released`; `release` returns whether the mode was released.
