 ...
 *          [--dist <dir>] [--expect open|closed] [--label <prefix>]
...
async function scene(name, contextOptions, press) {
  ...
  // The badge proves the seeded rows reached the store before the sheet opens.
  await bell.locator('span[aria-hidden="true"]').filter({ hasText: String(NOTES.length) }).waitFor({ timeout: 15000 })
  // Boot ends with a hop from /chat to the new slot's own URL, and the popover
  // closes itself on any route change — so the bell must not be pressed until
  // that hop has landed.
