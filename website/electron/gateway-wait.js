    ...
    if (failure.remoteHost) {
      ...
      if (failure.localStartFailed) {
        ...
        return "Starting a local gateway here did not finish: the restarted app "
          + "never served one. The setting stays on, so launching Kiro Crew again "
          + `tries again on a free port. ${reachTheCrew}`;
      }
      if (failure.localStartPortBusy) {
        ...
        return `Starting a local gateway here did not begin: port `
          + `${failure.localStartPortBusy} is already served by something else, so `
          + "restarting would not have produced a gateway this app can tell apart "
          + `from it. Stop whatever is on port ${failure.localStartPortBusy} and `
          + `choose Start Local Gateway again. The setting stays on. ${reachTheCrew}`;
      }
