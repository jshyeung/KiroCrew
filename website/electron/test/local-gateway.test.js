...
test("client-only: the local-start offer routes through a re-exec on a crew's port", () => {
  ...
  // that did not, and spends the one-shot button on a condition outside this app.
  ...
  // The record outlives each attempt, so the failure that withdraws the button
  // must also clear an earlier busy port, or the message keeps pointing at a
  // control this failure has just removed.
  assert.match(
    source,
  ...
  // reads must not still advertise it -- and the reopened wait reuses this same
  // record object rather than building a new one, so the attempted fact has to be
  // written here too or the message cannot tell a failed attempt from an app that
  // never offered the restart at all.
  assert.match(
    source,
    /localStartRelaunchFailed = true;[\s\S]*?gatewayStartFailure\.canStartHere = false;\n\s*gatewayStartFailure\.localStartFailed = true;/,
  );
  ...
  // Both ways the button closes leave canStartHere false, so the record must also
  // say whether a restart already ran, or the message cannot tell them apart.
...
test("launch port: an unusable KIROCREW_PORT takes the normal decision, not the bare default", () => {
  ...
  assert.ok(start > 0 && end > start, "resolvePort must precede its own call site");
  ...
  assert.match(body, /console\.warn\('Invalid KIROCREW_PORT=/);
  assert.match(body, /return selectLaunchPort\(\{/);
  // Only a usable value short-circuits selection.
  assert.match(body, /if \(!isNaN\(parsed\) && parsed >= 1 && parsed <= 65535\) return parsed;/);
  ...
  const migrateAt = source.indexOf("migrateRemoteHostConfig(store, CONFIGURED_PORT)");
  assert.ok(migrateAt > 0, "main must migrate the legacy host by the configured port");
