...
function migrateRemoteHostConfig(store, configuredPort) {
  ...
  if (legacy && Object.keys(store.get("remoteHosts") || {}).length === 0) {
    ...
    // A legacy entry names a host and no port, so the key chosen here is what
    // makes that crew visible at all -- and selection is the reader that needs to
    // see it, which is why this runs first and derives the key the same way.
    // `remoteHosts` is empty on this path by the guard above, so selection has no
    // crew to weigh and reduces to exactly this: the configured port when it is
    // selectable, the fallback otherwise.
    const port = isSelectablePort(configuredPort)
      ? configuredPort
      : fallbackLocalPort(store);
