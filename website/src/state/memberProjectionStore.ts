 ...
 *
 * It is sized against the SERVER's bound, not picked for comfort: an app may
 * hold `MAX_PROJECTION_KEYS_PER_UNIT` (200) projection keys on one unit, and a
 * teardown withdraws every key it published — so a cap below that evicts the
 * oldest markers during exactly the event the record exists to survive, and a
 * roster read already in flight then restores those cards. Keep this at or above
 * the server's per-app-per-unit key limit; lowering it re-opens that hole.
 */
const MAX_WITHDRAWN_KEYS_PER_SLUG = 200
...
export class MemberProjectionStore {
  ...
  apply(
    ...
    if (isTeardown) {
      ...
      byKey?.delete(key)
      // RECORD the withdrawal. Removing the row is what leaves a later baseline
      // unguarded: higher-seq-wins compares against a held row, and there is no
      // longer one to compare against, so a roster answer that was already in
      // flight when this landed would seed the card back. The write id says when
      // the withdrawal happened, which is the only thing that can tell such an
      // answer from one the server produced afterwards.
