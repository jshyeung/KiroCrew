 ...
 * in the stored order -- a different list than the one the person chose -- and
 * the page says so on an `ErrorNotice` above the card, with the server's own
 * words and the agent hand-off, the way the sidebar reports the same failure over
 * its own tree. The card's own ordering is covered in FolderSuggestionCard.test.tsx.
 */
...
describe('ChatPage folder-suggestion card: the folder order behind its options', () => {
  ...
  it('says the order could not be read, above the card drawn in the stored order', async () => {
    ;(api.kirocrewConfig as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('gateway restarting'))
    await renderPage(makeStore())
    // The page's notice by its own id: the sidebar in this tree reports the same
    // failure over its tree (`folder-order-unavailable`), and that one is the
    // sidebar test's to assert.
    ...
    // The hand-off stays on: the composer draft is persisted per slot and the
    // suggestion lives in the store, so opening a chat about the failure loses
    // nothing here.
    ...
    expect(optionOrder()).toEqual(['Work', 'Home'])
    // The notice sits with the card it explains, above it.
