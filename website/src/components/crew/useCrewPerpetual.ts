...
export interface CrewPerpetualOptions {
  /** Keep the registry's floor refetch and the roster re-read running while a
   *  crew is on screen (the editor's default). The Crewmates page passes
   *  `false`: its block already re-renders from the pushed `wake` projection,
   *  so a second poll there would only spend requests. */
  poll?: boolean
}
