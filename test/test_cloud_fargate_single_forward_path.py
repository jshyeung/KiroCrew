...
``cloud.ssm.build_port_forward_argv`` is the only argv builder, and
``cloud.ssm.open_port_forward`` is the only spawner carrying
``assert_human_action``. The instances layer spawns its own supervised child, so it
is reached by the argv census rather than the spawner census -- which is why both
exist here.
