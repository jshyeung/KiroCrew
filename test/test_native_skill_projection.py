...
def _prune_probe(monkeypatch, blocked=frozenset()):
    """Record every candidate the walk classifies; answer "leased" for *blocked*.

    The lease probe is the first per-candidate cost in the loop, so the names it
    sees ARE the candidates this call examined. Answering "leased" pins an entry
    unreclaimable without inventing a live projection for it.
    """
    seen = []
...
def test_a_backlog_cannot_stretch_the_locked_section_past_this_calls_budget(
    ...
    monkeypatch.setattr(projection, "_PRUNE_MAX_CANDIDATES_PER_RUN", 8)

    assert projection.prepare_native_skill_projection(project) is not None

    survivors = sum(1 for p in backlog if p.exists())
    assert survivors >= len(backlog) - 8, "the walk ran past this call's candidate budget"
    assert survivors < len(backlog), "the walk made no progress at all"
...
def test_the_walk_starts_where_the_rotation_points(native_tree, monkeypatch):
    ...
    monkeypatch.setattr(projection, "_PRUNE_MAX_CANDIDATES_PER_RUN", 1)
    seen = _prune_probe(monkeypatch, blocked={p.stem for p in order})
...
def test_an_unreclaimable_prefix_cannot_hide_the_backlog_behind_it(native_tree, monkeypatch):
    ...
    monkeypatch.setattr(projection, "_PRUNE_MAX_CANDIDATES_PER_RUN", 4)
    ...
    _prune_probe(monkeypatch, blocked={p.stem for p in pinned})
