...
class TestReservations:
    ...
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    @pytest.mark.parametrize("must_persist", [True, False])
    async def test_the_row_is_on_disk_before_apply_mode_returns(
        self, audits, session_map, must_persist
    ):
        """Both forms await the map's debounced write before anything is published
        past the mark -- header, hook, notice, return -- so a fresh map reads the row
        back with no flush of the test's own. Mutation: skip the ``aflush`` -- the
        fresh map reads no flag."""
        sm = session_map()
        applied = await privacy_mode.apply_mode(
            privacy_mode.MODE_INCOGNITO,
            _TG_KEY,
    ...
    async def test_a_failed_flush_refuses_and_takes_the_mark_back(
        ...
        that is a refusal -- the mark is taken back, the flag cleared, the user
        told -- and the trail reads ``allowed`` (the mark landed, audited before any
        await) then ``denied``. The best-effort form keeps the mark and carries on,
        the safe direction, logging the write it is still owed. Mutation: do not
        await the flush -- the strict mode applies."""
        sm = session_map()
        ...
        assert sm.get_flag(_TG_KEY, "incognito") is False
        assert rec.notices == [privacy_mode.refusal_notice("incognito", "persist_failed")]
        assert [e["outcome"] for e in audits] == ["allowed", "denied"]
        # The best-effort form: marked, told ON, the failed write logged not raised.
        applied = await privacy_mode.apply_mode(
            privacy_mode.MODE_INCOGNITO,
            _TG_KEY,
            source="telegram",
        ...
        assert rec.notices[-1] == privacy_mode.NOTICE_INCOGNITO
