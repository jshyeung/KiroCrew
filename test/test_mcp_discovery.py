...
class TestListServers:
    ...
    def test_non_boolean_disabled_warns_once_and_rearms_after_a_fix(
        ...
        caplog.clear()
        # Fixed: a real boolean is a real Disabled row -- no reason to add --
    ...
    def test_invalid_disabled_ledger_retains_digests_never_the_config_text(
        ...
        per-server log line is retained too (the dashboard ring keeps it), so
        it carries the name cut to ``_LOG_NAME_MAX`` and the value's type and
        size -- not the name whole, and never the value."""
        name = "figma-" + "x" * 300
        value = "false" * 100
        self._seed_shared_scope(
        ...
        assert name[: mcp_discovery._LOG_NAME_MAX] in line
        assert f"(+{len(name) - mcp_discovery._LOG_NAME_MAX} chars)" in line
