...
def test_the_crew_container_declares_a_stop_timeout_covering_the_whole_drain():
    ...
    The supervisor drains front, backend, then the writer, in that order. The writer's
    cycle carries the only copy of the turns the backend flushed on the way out, so a
    stop timeout shorter than the three windows together kills it, kills the supervisor
    with it, and the non-zero exit meant to report the loss is never delivered.
    """
    c = _container_constants()
    drain_sum = c["FRONT_DRAIN_SECS"] + c["BACKEND_DRAIN_SECS"] + c["SIDECAR_DRAIN_SECS"]

    assert container(td.task_definition_document(spec()))["stopTimeout"] >= drain_sum


def test_the_stop_timeout_matches_the_sum_the_image_declares():
    ...
    stops them drifting: change a drain window without changing this and the pin reds.
    The sum is recomputed here rather than read by name, because the image derives its
    own total from the same three windows and a derived name is not a literal.
    """
    c = _container_constants()
    drain_sum = c["FRONT_DRAIN_SECS"] + c["BACKEND_DRAIN_SECS"] + c["SIDECAR_DRAIN_SECS"]

    assert td.CREW_STOP_TIMEOUT_SECS == int(drain_sum)
