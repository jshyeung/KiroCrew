...
class TestConsentedDownloadRequiresOwnerIdentity:
    ...
    def test_the_refused_entry_says_which_conjunct_refused(self):
        ...
        Reaching the refusal with the class granted means identity is what failed,
        so the entry has to separate "the scanner held this back" from "another
        principal reached for a file your grant covers" -- the second read as the
        first tells the owner their own grant lapsed. Structural for the same
        reason as the conjunct tests above: an aiohttp fixture carrying a forged
        non-owner identity would pin the harness rather than the route, and the
        entry's own text is asserted behaviourally against the helper.

        Both flagged branches of the download leg are checked: the text scan and
        the binary-content scan each refuse, each can be reached with the class
        granted, and an entry that discriminated on only one of them would leave
        the other reading as a scanner hold-back.
        """
        import inspect
        ...
        # The selector is the GRANT, which is what makes the two rows discriminate:
        # inside these branches a granted class can only mean the owner conjunct failed.
        assert src.count("cross_principal = granted") == 2
        assert (
