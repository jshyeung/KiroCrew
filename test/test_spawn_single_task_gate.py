...
The solo-spawn gate (#11710, widened in #12203) validated reason SHAPE, so the
model always had a passing token: local audit showed 33 of 34 one-task calls
let through. It was removed. These tests pin what replaced it:

* the tool descriptions carry the concrete "one task: do it yourself" default
  and no longer advertise a reason vocabulary;
...
* the gateway no longer refuses or audits a lone spawn, and still reports
...
from kiro_crew.parent_work import parent_work_supported
